# from https://github.com/jaidevd/jaidevd.github.io/blob/source/blog/posts/code/weighted_loss_unet.py
# coding: utf-8
...
def make_weighted_loss_unet(is_training=True):
    ...
    up6 = L.Conv2DTranspose(
        512, 2, strides=2, kernel_initializer="he_normal", padding="same"
    )(conv5)
    ...
    up7 = L.Conv2DTranspose(
        256, 2, strides=2, kernel_initializer="he_normal", padding="same"
    )(conv6)
    ...
    up8 = L.Conv2DTranspose(
        128, 2, strides=2, kernel_initializer="he_normal", padding="same"
    )(conv7)
    ...
    up9 = L.Conv2DTranspose(
        64, 2, strides=2, kernel_initializer="he_normal", padding="same"
    )(conv8)
    ...
    c10 = L.Conv2D(n_classes, 1, activation="softmax", kernel_initializer="he_normal")(
        conv9
    )
    ...
    # aggregation.
    if is_training:
        c11 = L.Lambda(lambda x: x / tf.reduce_sum(x, len(x.get_shape()) - 1, True))(
            c10
        )
        c11 = L.Lambda(lambda x: tf.clip_by_value(x, _epsilon, 1.0 - _epsilon))(c11)
